...
class ServicoRepositoryAdapterTest {
    ...
    fun `salvar deve persistir e mapear de volta`() {
        `when`(jpaRepository.save(anyObject())).thenAnswer {
            it.getArgument<ServicoJpaEntity>(0)
        }
    ...
    fun `buscarPorId deve mapear quando presente`() {
        `when`(jpaRepository.findById(ordemServico.id.valor))
    ...
    fun `listarPorStatus deve filtrar e mapear lista`() {
        ...
        `when`(jpaRepository.findByStatus(status))
    ...
    fun `listarPorCliente deve filtrar e mapear lista`() {
        ...
        `when`(jpaRepository.findByClienteId(clienteId.valor))
            .thenReturn(listOf(jpa))
    ...
    fun `listarPorVeiculo deve filtrar e mapear lista`() {
        ...
        `when`(jpaRepository.findByVeiculoId(veiculoId.valor))
            .thenReturn(listOf(jpa))

        val resultado = adapter.listarPorVeiculo(veiculoId)

        assertEquals(listOf(ordemServico), resultado)
        verify(jpaRepository).findByVeiculoId(veiculoId.valor)
    }
    ...
    fun `listarPorDataAberturaEntre deve filtrar e mapear lista`() {
        ...
        `when`(jpaRepository.findByDataAberturaBetween(inicio, fim))
            .thenReturn(listOf(jpa))
