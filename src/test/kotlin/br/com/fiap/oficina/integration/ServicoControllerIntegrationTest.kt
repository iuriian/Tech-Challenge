...
class ServicoControllerIntegrationTest : AbstractIntegrationTest() {
    ...
    private fun criarServico(): ServicoDto {
        val response = mockMvc.post("/servicos") {
    ...
    fun `deve criar servico com status inicial RECEBIDA e recupera-lo por id`() {
        ...
        mockMvc.get("/servicos/${criado.id}")
            .andExpect {
                status { isOk() }
    ...
    fun `deve calcular o orcamento somando preco de venda vezes quantidade`() {
        ...
        mockMvc.get("/servicos/${criado.id}/orcamento")
            .andExpect {
                status { isOk() }
    ...
    fun `deve avancar o status seguindo o fluxo da maquina de estados`() {
        ...
        mockMvc.patch("/servicos/${criado.id}/avancar")
            .andExpect {
                status { isOk() }
    ...
    fun `deve listar servicos por cliente`() {
        ...
        mockMvc.get("/servicos/cliente/{clienteId}", clienteId())
            .andExpect {
                status { isOk() }
                jsonPath("$") { isArray() }
    ...
    fun `deve retornar metricas de tempo medio de execucao`() {
        ...
        mockMvc.get("/servicos/metricas/tempo-medio")
            .andExpect {
                status { isOk() }
    ...
    fun `deve deletar servico retornando 204`() {
        ...
        mockMvc.delete("/servicos/${criado.id}")
