...
class OrdemServicoControllerIntegrationTest : AbstractIntegrationTest() {
    @Autowired
    ...
    private fun clienteId(): UUID = clienteRepository
        .buscarPorDocumento(
            "39053344705",
        )!!
        .id
    ...
    private fun veiculoId(): UUID = veiculoRepository
        .buscarPorPlaca(
            "ABC1D23",
        )!!
        .id
    ...
    private fun pecaId(): UUID = pecaRepository
        .buscarAtivoPorCodigo(
            "PEC001",
        )!!
        .id
        .valor
    ...
    private fun ordemServicoJson(
        ...
        OrdemServicoRequest(
            descricao = descricao,
            funcionarioId =
            "3f5f33b0-4f1f-4a76-9ef8-1dc8b8d1a1b3",
    ...
    private fun criarOrdemServico(): OrdemServicoResponse {
        val response =
            mockMvc
                .post("/servicos") {
                    contentType =
    ...
    fun `deve criar ordem de servico com status inicial RECEBIDA e recupera-la por id`() {
        val response =
            mockMvc
                .post("/servicos") {
                    ...
                    jsonPath("$.descricao") {
                        value("Revisão completa")
                    }
                }.andReturn()

        val ordemServicoCriada: OrdemServicoResponse =
        ...
        mockMvc
            .get(
                ...
                jsonPath("$.id") {
                    value(
                        ordemServicoCriada.id.toString(),
                    )
                }
    ...
    fun `deve calcular o orcamento somando preco de venda vezes quantidade`() {
        ...
        mockMvc
            .get(
                "/servicos/${ordemServicoCriada.id}/orcamento",
            ).andExpect {
                status {
                    isOk()
                }
    ...
    fun `deve avancar o status seguindo o fluxo da maquina de estados`() {
        ...
        mockMvc
            .patch(
                "/servicos/${ordemServicoCriada.id}/avancar",
            ).andExpect {
                status {
                    isOk()
                }
    ...
    fun `deve retornar 422 em transicao de status nao permitida`() {
        ...
        mockMvc
            .patch(
                "/servicos/${ordemServicoCriada.id}/status",
            ) {
                contentType =
                    MediaType.APPLICATION_JSON
                content = dto
