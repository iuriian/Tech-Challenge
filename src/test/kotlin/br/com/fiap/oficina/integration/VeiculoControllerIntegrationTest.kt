...
class VeiculoControllerIntegrationTest : AbstractIntegrationTest() {
    ...
    private fun motoristaSeedId(): UUID =
    ...
    fun `deve criar veiculo e recupera-lo por id`() {
        val response = mockMvc.post("/veiculos") {
        ...
        mockMvc.get("/veiculos/${criado.id}")
            .andExpect {
                status { isOk() }
                jsonPath("$.placa") { value("TST1A23") }
    ...
    fun `deve buscar veiculo de seed por placa`() {
        mockMvc.get("/veiculos/placa/{placa}", "ABC1D23")
            .andExpect {
                status { isOk() }
    ...
    fun `deve atualizar um veiculo existente`() {
        ...
        val atualizado = objectMapper.writeValueAsString(
            criado.copy(modelo = "Civic Touring")
        )

        mockMvc.put("/veiculos/${criado.id}") {
            contentType = MediaType.APPLICATION_JSON
            content = atualizado
        }.andExpect {
            status { isOk() }
    ...
    fun `deve remover veiculo retornando 204`() {
        ...
        mockMvc.delete("/veiculos/${criado.id}")
